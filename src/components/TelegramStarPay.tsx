  ...
  const handleOnChainPayment = async (
    ...
    try {
      if (method === 'usdc') {
        ...
        if (result.success) {
          const tx =
            getField<string>(result, 'txHash') ??
            getField<string>(result, 'transactionHash')
          const amt =
            getField<number>(result, 'amount') ??
            getField<number>(result, 'value')

          setBanner({
            type: 'success',
            message: `USDC payment submitted${tx ? ` (tx: ${formatAddress(tx, 6, 6)})` : ''}. Waiting for confirmation.`,
          })

          // 提交到后端进行验证与记录（非阻塞）
          try {
            const price2 = getPriceByMethod(item, 'usdc')
            const payload = {
              itemId: item.itemId,
              paymentMethod: 'usdc' as const,
              txHash: tx,
              payer: wallet.address ?? null,
              payee: paymentAddress,
              amount: amt ?? price2?.amount,
              assetId: getField<number>(price2, 'assetId'),
              assetName:
                getField<string>(price2, 'label') ??
                getField<string>(price2, 'assetName'),
              assetIcon: getField<string>(price2, 'assetIcon'),
              listedAmount: price2?.amount,
              quantity: 1,
            }
            ...
            console.warn('submitPurchase (USDC) failed', err)
          }
        } else {
          throw new Error(
            getField<string>(result, 'error') ??
              result.error ??
              'USDC payment failed'
          )
        }
      ...
      if (tonResult.success) {
        const tx =
          getField<string>(tonResult, 'txHash') ??
          getField<string>(tonResult, 'transactionHash')
        const amt =
          getField<number>(tonResult, 'amount') ??
          getField<number>(tonResult, 'value')

        setBanner({
          type: 'success',
          message: `Submitted TON payment to ${formatAddress(paymentAddress, 4, 6)}${tx ? ` (tx: ${formatAddress(tx, 6, 6)})` : ''}. Waiting for confirmation.`,
        })

        // 提交到后端进行验证与记录（非阻塞）
        try {
          const price3 = getPriceByMethod(item, 'ton')
          const payload = {
            itemId: item.itemId,
            ...
            payer: wallet.address ?? null,
            payee: paymentAddress,
            amount: amt ?? price3?.amount,
            assetId: getField<number>(price3, 'assetId'),
            assetName:
              getField<string>(price3, 'label') ??
              getField<string>(price3, 'assetName'),
            assetIcon: getField<string>(price3, 'assetIcon'),
            listedAmount: price3?.amount,
            quantity: 1,
          }

          await submitPurchase(payload)
        } catch (err) {
          console.warn('submitPurchase (TON) failed', err)
        }
      } else {
        throw new Error(
          getField<string>(tonResult, 'error') ??
            tonResult.error ??
            'TON payment not sent'
        )
      }
    } catch (error) {
      const message =
